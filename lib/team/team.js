...
*/
	...
	createCardHTML(employee) {
		...
		cardSubtitle.classList.add("card-subtitle", "mb-2", "role");
		...
		let textProperties = Object.keys(employee.data)
			.filter(element => !(["name", "html"].includes(element)));
		for (const property of textProperties) {
			let span = document.createElement("span");
			span.classList.add("card-text", property);
			if (property === "id") {
				span.innerHTML = `Employee ID: ${employee.data[property]}`;
			} else if (property === "email") {
				span.innerHTML = `<a href="mailto:${employee.data[property]}">${employee.data[property]}</a>`;
			} else if (property === "github") {
				span.innerHTML = `<a href="https://www.github.com/${employee.data[property]}/">${employee.data[property]}</a>`;
			} else if (property === "officeNumber") {
				span.innerHTML = `Office Number: ${employee.data[property]}`;
			} else if (property === "school") {
				span.innerHTML = `School: ${employee.data[property]}`;
			} else {
		...
		if (employee.parent != null && employee.parent.data != null) {
			...
			managerLink.href = `${employee.parent.data.id}.html`;
